  ...
  try {
    ...
    if (provider === "paystack") {
      ...
      if (payload.notification_status === "payment_successful" && payload.transaction_status === "success") {
        const grossAmount = Number(payload.amount_paid) || 0;
        const accountNumber = payload.virtual_account_number || payload.account_number || "";
        const txnRef = payload.transaction_id || payload.reference || `XX-${Date.now()}`;
        ...
        if (grossAmount > FUNDING_FEE) {
          ...
          if (userId) {
            ...
            if (!existing) {
              ...
              console.log(`[webhook] XixaPay: paid ₦${grossAmount}, fee ₦${FUNDING_FEE}, credited ₦${netAmount} to user ${userId}`);
            } else {
              console.log(`[webhook] XixaPay: duplicate ref ${txnRef}, skipping`);
            }
          } else {
            console.warn(`[webhook] XixaPay: could not identify user for account ${accountNumber}`);
          }
        } else if (grossAmount > 0 && grossAmount <= FUNDING_FEE) {
