  ...
  if (logo) {
    return (
      <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center overflow-hidden", logo.bg, className)}>
        <img src={logo.src} alt={networkName} className="w-10 h-10 object-contain" />
      </div>
    );
  }

  return (
    <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center text-sm font-black bg-purple-100 text-purple-700", className)}>
      {networkName[0]}
    </div>
  );
};
